...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  void _loadUserData() {
    ...
    if (user != null) {
      ...
      _selectedRole =
          _familyRoles.contains(user.roleInFamily)
              ? user.roleInFamily
              : 'Individual';

      // Determine tracking preference based on existing data
      if (user.familyMembers.isEmpty && user.dependencies.isEmpty) {
        _selectedTrackingPreference = 'Individual';
        _individualIncome = user.totalFamilyIncome;
        _individualIncomeController.text = user.totalFamilyIncome.toString();
      } else if (user.dependencies.isEmpty && user.familyMembers.length <= 1) {
      ...
      for (var member in user.familyMembers) {
        ...
        _familyRelationships.add(
          _availableFamilyRelationships.contains(member.relationship)
              ? member.relationship
              : 'Son',
        );
      }
      ...
      for (var dependency in user.dependencies) {
        ...
        _dependencyTypes.add(
          _availableDependencyTypes.contains(dependency.type)
              ? dependency.type
              : 'Housewife',
        );
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      if (_selectedTrackingPreference == 'Individual') {
        totalFamilyIncome = _individualIncome;
      } else {
    ...
    return Column(
      children: [
        ...
        DropdownButtonFormField<String>(
          value:
              _familyRoles.contains(_selectedRole)
                  ? _selectedRole
                  : 'Individual',
          decoration: InputDecoration(
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  DropdownButtonFormField<String>(
                    value:
                        _availableFamilyRelationships.contains(
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  DropdownButtonFormField<String>(
                    value:
                        _availableDependencyTypes.contains(
